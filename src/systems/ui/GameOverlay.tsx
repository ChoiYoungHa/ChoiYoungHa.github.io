...
export function GameOverlay({ session, loading, backend, preset, projector, bgUrl }: GameOverlayProps) {
  ...
  return (
    <div data-game-overlay="true" style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      ...
      {scene === 'shop' && snapshot.shopOpen && game.jobId !== null && <ShopPanel state={{ jobId: game.jobId, meso: game.meso, inventory: game.inventory }} selectedItemId={snapshot.selectedShopItemId} ipMode={game.ipMode} onSelect={(selectedItemId) => session.enqueueInput({ selectedItemId })} onPurchase={(result) => { if (result.ok) session.enqueueInput({ confirm: true, selectedItemId: snapshot.selectedShopItemId ?? undefined }) }} onAfterPurchase={() => undefined} onLeave={() => session.enqueueInput({ cancel: true })} />}
