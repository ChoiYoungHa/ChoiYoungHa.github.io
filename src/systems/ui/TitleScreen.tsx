...
export function TitleScreen({ bgUrl, loading, backend, preset, ipMode, onStart }: TitleScreenProps) {
  ...
  return (
    <section
      ...
      onPointerDown={view.canStart ? (e) => { if (e.button === 0) onStart() } : undefined}
      style={{ position: 'absolute', inset: 0, overflow: 'hidden', color: HUD_TOKENS.colors.text, fontFamily: HUD_TOKENS.fontFamily, cursor: view.canStart ? 'pointer' : 'default', pointerEvents: 'auto' }}
    >
      ...
      <div style={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '15vh', boxSizing: 'border-box', textAlign: 'center' }}>
        ...
        <button type="button" disabled={!view.canStart} onPointerDown={(e) => e.stopPropagation()} onClick={onStart} style={{ marginTop: 24, minWidth: 180, border: `1px solid ${HUD_TOKENS.colors.border}`, borderRadius: 8, padding: '12px 24px', background: view.canStart ? '#80652f' : '#313238', color: view.canStart ? '#fff7dc' : '#85858b', cursor: view.canStart ? 'pointer' : 'not-allowed', font: 'inherit', fontWeight: 700 }}>{view.startLabel}</button>
      </div>
