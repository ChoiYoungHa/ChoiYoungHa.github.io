 ...
 * Circle.radius 는 heroTree.resolveCollision 규약대로 **플레이어 반경을 포함**한 값(소품 반경×스케일 + playerRadius).
 */
...
export function isDressingBlocked(x: number, z: number): boolean {
  for (const prop of DRESSING_PROPS) if (Math.hypot(x - prop.position[0], z - prop.position[1]) < DRESSING_CLEARANCE + prop.collisionRadius * prop.scale) return true
  const g = GROUND
