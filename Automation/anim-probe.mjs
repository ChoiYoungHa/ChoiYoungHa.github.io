...
await key('keyDown', 'KeyW'); await sleep(1400); const shotWalk = await send('Page.captureScreenshot', { format: 'png' }); await writeFile('Docs/qa/anim-probe/walk-hold.png', Buffer.from(shotWalk.data, 'base64')); await key('keyUp', 'KeyW'); await sleep(200)
await key('keyDown', 'KeyS'); await sleep(1400); { const sh = await send('Page.captureScreenshot', { format: 'png' }); await writeFile('Docs/qa/anim-probe/walk-back.png', Buffer.from(sh.data, 'base64')) } await key('keyUp', 'KeyS'); await sleep(600)
...
await key('keyDown', 'KeyW'); await sleep(1400); { const sh = await send('Page.captureScreenshot', { format: 'png' }); await writeFile('Docs/qa/anim-probe/walk-again.png', Buffer.from(sh.data, 'base64')) } await key('keyUp', 'KeyW'); await sleep(300)
